...
<div id="product-images" class="panel product-tab">
	...
	<table class="table tableDnD" id="imageTable">
		<thead>
			<tr class="nodrag nodrop"> 
				...
				<th class="fixed-width-xs"><span class="title_box">{l s='History'}</span></th>
				<th></th> <!-- action -->
			</tr>
		</thead>
		<tbody id="imageList">
	...
	<table id="lineType" style="display:none;">
		<tr id="image_id">
			...
			<td class="history">
				<a href="#">
					<i class="icon-chevron-sign-up icon-2x historied"></i>
				</a>
			</td>
			<td>
	...
	<script type="text/javascript">
		...
		function imageLine(id, path, position, cover, history, shops, legend)
		{
			line = $("#lineType").html();
			...
			line = line.replace(/icon-chevron-sign-up/g, history);
		...
		$(document).ready(function(){
			...
			{foreach from=$images item=image}
				...
				if (assoc != {literal}"{"{/literal})
				{
					...
					assoc = false;
				imageLine({$image->id}, "{$image->getExistingImgPath()}", {$image->position}, "{if $image->cover}icon-check-sign{else}icon-check-empty{/if}","{if $image->history}icon-chevron-sign-down{else}icon-chevron-sign-up{/if}", assoc, "{$image->legend[$default_language]|@addcslashes:'\"'}");
			{/foreach}
			{literal}
			...
			$('.historied').die().live('click', function(e)
			{
				e.preventDefault();
				id = $(this).parent().parent().parent().attr('id');
				$("#imageList .history i").each( function(i){
