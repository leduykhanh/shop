...
//Lee added
	public function ajaxProcessUpdateHistory()
	{
		Image::deleteHistory((int)Tools::getValue('id_product'));
		$img = new Image((int)Tools::getValue('id_image'));
		$img->history = 1;
		...
		@unlink(_PS_TMP_IMG_DIR_.'product_mini_'.(int)$img->id_product.'_'.$this->context->shop->id.'.jpg');
		if ($img->update())
			$this->jsonConfirmation("History image updated");
		else
			$this->jsonError(Tools::displayError('An error occurred while attempting to move this picture.'));
	}
	...
	*/
	...
	public function initFormInformations($product)
	{
		...
		*/
		...
		$data->assign('PS_PRODUCT_SHORT_DESC_LIMIT', Configuration::get('PS_PRODUCT_SHORT_DESC_LIMIT') ? Configuration::get('PS_PRODUCT_SHORT_DESC_LIMIT') : 4000);
		$this->tpl_form_vars['product'] = $product;
		$this->tpl_form_vars['custom_form'] = $data->fetch();
	}
